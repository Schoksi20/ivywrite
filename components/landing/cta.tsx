  ...
  return (
    <section className="py-20 md:py-32 px-6 md:px-12 flex flex-col items-center text-center bg-surface border-t border-border" id="cta">
      ...
      <h2 className="text-4xl md:text-5xl font-bold tracking-tight leading-tight max-w-2xl text-heading mb-5">
        Get your Ivy-level SOP in 72 hours or less
      </h2>
      ...
      <div className="inline-flex flex-col sm:flex-row items-center gap-8 bg-card border-2 border-accent/20 rounded-2xl p-8 sm:px-12 mb-8 shadow-lg">
        <div className="text-6xl font-bold tracking-tight leading-none text-accent">
          <sup className="text-3xl align-super">&#8377;</sup>999
        </div>
        ...
        {checks.map((c, i) => (
          <div key={i} className="flex items-center gap-2 text-sm text-body font-medium">
            <svg viewBox="0 0 14 14" fill="none" className="w-4 h-4 shrink-0">
              <path d="M2 7l3.5 3.5L12 3" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            {c}
          </div>
        ))}
      ...
      <Link
        href="/questionnaire"
        className="bg-accent text-white text-lg font-semibold px-10 py-4 rounded-lg hover:bg-accent/90 transition-colors inline-flex items-center gap-3 shadow-lg"
      >
        Get my SOP now
